    ...
    >
        ...
        <div class="idm_itodotabslist_list_box">
          <component
            :is="
              ...
              propData.useTableTabArrayStr.split(',').indexOf(item.key + '') >
                -1
                ? 'table-list-module'
                : 'diverse-list-module'
            "
          ...
          <label
            ...
            :class="{
              'ant-tabs-tab-remind-reddot':
                propData.remindShowDot && item.key != activeTab,
            }"
            >{{
      ...
      <div class="idm_itodotabslist_tabbarextra_box" slot="tabBarExtraContent">
        ...
        <template v-for="(bitem, bindex) in extraBtnList || []">
          <div
            v-if="bitem.showStatus"
            @click="tabBarExtraClickHandle(bitem, bindex)"
            :key="bindex"
          >
  ...
  methods: {
    ...
    initBaseAttrToModule() {
      ...
      that.propData.extraBtnList &&
        that.propData.extraBtnList.forEach(
          (item) => (item.showStatus = item.showType == "default")
        );
        ...
        that.allDynamicAttrList.forEach((item) => {
            ...
            try {
              ...
              if (tabsData instanceof Array) {
                ...
                tabsData.forEach((fitem) => {
                  newTabsList.push({
                    key: that.getExpressData(
                      "data",
                      that.propData.dynamicKeyName,
                      fitem
                    ),
                    tab: that.getExpressData(
                      "data",
                      that.propData.dynamicShowName,
                      fitem
                    ),
                    cnt: 0,
          ...
          that.extraBtnList &&
            that.extraBtnList.forEach((eitem) => {
              if (
                eitem.showType == "dynamic" &&
                eitem.dynamicAttrKeyName == item.attrCode
              ) {
                //dynamicAttrKeyName
                //dynamicCompareType
                if (eitem.dynamicCompareType == "result") {
                  //结果值比较
                  eitem.showStatus =
                    eitem.dynamicCompareResult == item.attrData;
      ...
      if (
        ...
        (that.allTabList &&
          (that.allTabList.length == 0 ||
            (that.allTabList.length == 1 &&
              Object.keys(that.allTabList).length == 0)))
      ) {
        return;
      }
      ...
      IDM.controlcenter &&
        IDM.controlcenter
          .getModuleAttrInstance({
            marketModuleId: this.moduleObject.comId,
            pageId:
              IDM.broadcast && IDM.broadcast.pageModule
                ? IDM.broadcast.pageModule.id
                : "",
            packageid: this.moduleObject.packageid,
          })
          .then((res) => {
    ...
    convertTabAttrToStyleObject() {
      ...
      window.IDM.setStyleToPageHead(
        this.moduleObject.id + " .ant-tabs-tab-remind",
        styleObject
      );
      ...
      window.IDM.setStyleToPageHead(
        this.moduleObject.id + " .idm_itodotabslist_tabbarextra_box>*",
        extraBtnStyleObject
      );
      window.IDM.setStyleToPageHead(
        this.moduleObject.id + " .idm_itodotabslist_tabbarextra_box>* svg",
        extraBtnSvgStyleObject
      );
    },
    /**
    ...
    convertListAttrToStyleObject() {
      ...
      if (this.propData.diverseDefaultLineHeight) {
        diverseStyleObject["height"] =
          this.propData.diverseDefaultLineHeight + "px";
        diverseStyleObject["line-height"] =
          this.propData.diverseDefaultLineHeight + "px";
      ...
      if (
        this.propData.diverseHoverBgColor &&
        this.propData.diverseHoverBgColor.hex8
      ) {
        diverseHoverStyleObject["background-color"] =
          this.propData.diverseHoverBgColor.hex8;
      }
      if (this.propData.diverseHoverShadow) {
        diverseHoverStyleObject["box-shadow"] =
          this.propData.diverseHoverShadow;
      }
      ...
      if (this.propData.diverseLoadingFont) {
        ...
        diverseLoadStyleObject["font-size"] =
          element.fontSize + element.fontSizeUnit;
      ...
      if (
        this.propData.tableTitleBgColor &&
        this.propData.tableTitleBgColor.hex8
      ) {
        tableTitleStyleObject["background-color"] =
          this.propData.tableTitleBgColor.hex8;
      }
      window.IDM.setStyleToPageHead(
        this.moduleObject.id +
          " .idm-diverse-list-module-box .idm-dlm-item-static",
        diverseStyleObject
      );
      window.IDM.setStyleToPageHead(
      ...
      window.IDM.setStyleToPageHead(
        this.moduleObject.id +
          " .idm-diverse-list-module-box .idm-dlm-item-body",
        tableBodyStyleObject
      );
      window.IDM.setStyleToPageHead(
        this.moduleObject.id +
          " .idm-diverse-list-module-row .idm-dlm-item-hover",
        diverseHoverStyleObject
      );
      window.IDM.setStyleToPageHead(
        this.moduleObject.id +
          " .idm-diverse-list-module-box .idm-dlm-load-style",
        diverseLoadStyleObject
      );
      window.IDM.setStyleToPageHead(
        this.moduleObject.id +
          " .idm-diverse-list-module-box>.idm-dlm-item-flex",
        tableTitleStyleObject
      );
    },
    /**
      ...
      this.$nextTick(function(){
        ...
        attrArray.forEach(item=>{
          if(this.propData.innerBox&&this.propData.innerBox[item]){
            if(this.propData.innerBox[item].indexOf("%")>-1){
              //用宽度计算出实际的px
              contentTBMarginNumber =contentTBMarginNumber+ (parseFloat(this.propData.innerBox[item].replace("%",""))/100)*tabContentWidth;
            }else{
              contentTBMarginNumber =contentTBMarginNumber+ parseFloat(this.propData.innerBox[item].replace("px",""))
            }
          }
        })
        //tabContent容器的内边距
        ...
        attrArray.forEach(item=>{
          if(this.propData.innerBox&&this.propData.innerBox[item]){
            if(this.propData.innerBox[item].indexOf("%")>-1){
              //用宽度计算出实际的px
              contentTBPaddingNumber =contentTBPaddingNumber+ (parseFloat(this.propData.innerBox[item].replace("%",""))/100)*tabContentWidth;
            }else{
              contentTBPaddingNumber =contentTBPaddingNumber+ parseFloat(this.propData.innerBox[item].replace("px",""))
            }
          }
        })
        ...
        attrArray.forEach(item=>{
          if(this.propData.innerBox&&this.propData.innerBox[item]){
            if(this.propData.innerBox[item].indexOf("%")>-1){
              //用宽度计算出实际的px
              tabInnerContentWidth =tabInnerContentWidth- (parseFloat(this.propData.innerBox[item].replace("%",""))/100)*tabContentWidth;
            }else{
              tabInnerContentWidth =tabInnerContentWidth- parseFloat(this.propData.innerBox[item].replace("px",""))
            }
          }
        })
        ...
        attrArray.forEach(item=>{
          if(this.propData.diverseDefaultBox&&this.propData.diverseDefaultBox[item]){
            if(this.propData.diverseDefaultBox[item].indexOf("%")>-1){
              //用宽度计算出实际的px
              tabInnerMarginNumber =tabInnerMarginNumber+ (parseFloat(this.propData.diverseDefaultBox[item].replace("%",""))/100)*tabInnerContentWidth;
            }else{
              tabInnerMarginNumber =tabInnerMarginNumber+ parseFloat(this.propData.diverseDefaultBox[item].replace("px",""))
            }
          }
        })
